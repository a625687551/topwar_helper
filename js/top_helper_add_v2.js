...
helper.pro.findPlayer={
    ...
    async _searchCity() {
        ...
        for (var i = 0; !this.stopping && !this.point && i < this.map.block_num; ++i) {  
            // 地图划分为block_num块,每块坐标范围86*156,部分重叠(每次请求服务器返回90*160)  普通地图坐标范围:[x:0-816, y:0-952]
    ...
    async _searchFields(pid) 
    {
        ...
        for(var i = 0; i < this.map.block_num; ++i) {
            var o = await this._getBlock(i)
            o && o.pointList && o.pointList.forEach(p=>(p.r && p.r.ownerId == pid || p.b && p.b.pid == pid) &&       
            // r:{pointType=4:普通田，=38:联盟田} b:{pointType=24:机械田} p:{pointType=1:基地}
...
helper.pro.avoidAttacked = {
    ...
    //---------------------------------------------------------------------------------------------
    async _charge() {
        if (UserData.getEnergy(1).Point >= 10) return 1
        var e = UserData.getItemList().find(e=>6 == e.Data.type && e.Amount > 0)
        return e && await send(RequestId.ITEM_USE, {amount:1, itemid:e.ItemId}) ? 2 : 0
    },
    ...
    async _gatherProtect(t) {
        ...
        for(var o, i = 0; i < n && s[i]; ++i) {
            o = await this._searchResource()
            o = o && await this._gather(this._tileInfo(o), Number(s[i]))
            r = o && o.marchInfo || r
        }
        return r
    }, 
    ...
    async _assembly(ti, preset) {
        if (await helper.battle.mutex.acquire(2000)) {
            try {
                var context = helper.battle.backup()
                Object.assign(new(__require("NWorldMapAssemblyEnemyComponent").default), {tileInfo:ti}).StartAssembly(null, 0)
    ...
    async _searchHammer() {
        var c = helper.pro.createAssembly.config && helper.pro.createAssembly.config[0]
        var o = c && await send(RequestId.WORLD_SEARCH_MONSTER, {groupType: c.id, pointType: c.type, minLevel: c.cmin, maxLevel: c.cmax})
        return o && o.point
    },
    ...
    async _assemblyProtect(t) {
        ...
        var s = t.toString(), r = !1
        for(var o, i = 0; i < n && s[i]; ++i) {
            o = await this._charge() 
            o = o && await this._searchHammer()
            o = o && await this._assembly(this._tileInfo(o), Number(s[i]))
            r = o && o.marchInfo && (o.marchInfo.repatriates=[], this.assemls.push(o.marchInfo)) || r
        }
        this.assemls.splice(0, this.assemls.length-8) 
        return r
    },

    async _repatriate(e) {
        for (var asseml of this.assemls) {
            if(asseml.marchId == e.marchId) {
                for(var member of e.members) {
                    member.marchId != asseml.marchId && !asseml.repatriates.find(t=>t.marchId == member.marchId) && (
                        asseml.repatriates.push(member),
                        send(RequestId.ALLIANCE_Assembly_Repatriation, {teamId: e.teamId, marchId: e.members[i].marchId})   //遣返
                    )
                }
            }
        }
    ...
    onAssemblyUpdate(e) {
        0 == e.marchState && this._repatriate(e)  //执行遣返动作
    },

    onMyMarchUpdate(e) {
        var o = JSON.parse(e)
        o && 4 == o.marchInfo.state && this.onAttacking()  //行军返回时执行被攻击处理流程
    },
    ...
    _onEvent() {
        __require("EventCenter").EVENT.on(__require("EventId").EventId.AttackingMarchUpdate, this.onAttacking, this)   //攻击事件
        __require("EventCenter").EVENT.on(__require("EventId").EventId.AssemblyUpdate, this.onAssemblyUpdate, this)   //集结数据变更事件
        __require("EventCenter").EVENT.on(__require("EventId").EventId.My_March_Update, this.onMyMarchUpdate, this)    //行军数据变更事件
    },

    _offEvent() {
        __require("EventCenter").EVENT.off(__require("EventId").EventId.AttackingMarchUpdate, this.onAttacking, this)   //攻击事件
        __require("EventCenter").EVENT.off(__require("EventId").EventId.AssemblyUpdate, this.onAssemblyUpdate, this)   //集结数据变更事件
        __require("EventCenter").EVENT.off(__require("EventId").EventId.My_March_Update, this.onMyMarchUpdate, this)    //行军数据变更事件
    },
    ...
    _render() {
        ...
        if (iframe && iframe.name == "avoid-attacked") {
            for (var history=[], i=this.history.length-1; i>=0; --i) {
                ...
                if (1 == e.type) {
                    history.push(timestr(e.time) + '  ' + e.name + '(' + e.uid + ')从' + e.x + ',' + e.y + '攻击, ' + (e.action==0 ? '未启动保护' : e.action==1 ? '护盾保护' : e.action==2 ? '迁城保护' : '集结保护'))  
                } 
                if (2 == e.type) {
                    history.push(timestr(e.time) + '  ' + e.name + '(' + e.uid + ')抵近(' + e.x + ',' + e.y + '), ' + (e.action==0 ? '未启动保护' : e.action==1 ? '护盾保护' : e.action==2 ? '迁城保护' : '集结保护'))
                }
            }
...
helper.pro.dailyGift = {
    async _execute() {
        ...
        if (o && o.time < ServerTime - UserData.timeReward.rewardTime) {
